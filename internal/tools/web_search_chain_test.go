...
func TestBuildChainFromStorage(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "explicit Parallel needs no secret and stays before DDG",
			tenantID:  uuid.New(),
			override:  `{"provider_order":["parallel"],"parallel":{"enabled":true,"max_results":3}}`,
			secrets:   map[string]string{},
			wantNames: []string{"parallel", "duckduckgo"},
			wantLen:   2,
		},
		{
